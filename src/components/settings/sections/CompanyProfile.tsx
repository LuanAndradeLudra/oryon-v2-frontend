...
export function CompanyProfile() {
  ...
  const [form, setForm] = useState({ name: '', email: '', timezone: '', language: '' })
  ...
  useEffect(() => {
    axios.get(`${API}/settings/company`).then((r) => {
      ...
      setTenant(mapped)
      setForm({ name: String(mapped.name ?? ''), email: String(mapped.email ?? ''), timezone: String(mapped.timezone ?? ''), language: String(mapped.language ?? '') })
    }).catch(() => {
