 ...
 *
 * TODA escrita é FUNCIONAL (`setCounts(prev => ...)`), e isso é o achado B1 do
 * Lince — terceira ocorrência da família neste épico. A versão anterior montava
 * o mapa novo ANTES do `await` e escrevia depois: duas resoluções sobrepostas
 * partiam do mesmo retrato e a última apagava a primeira. O gatilho é o próprio
 * polling (cada tique troca `campaigns`, e o POST de contagem é lento por
 * construção), e o sintoma é um número que PISCA E SOME — pior que nunca
 * aparecer, porque parece defeito de dado. O `pending` evita a CHAMADA
 * duplicada, não a ESCRITA perdida: são coisas diferentes.
 */
export function useAudienceCounts(campaigns: Campaign[]): Map<string, number> {
  ...
  const write = useCallback((id: string, count: number) => {
    setCounts((prev) => (prev.get(id) === count ? prev : new Map(prev).set(id, count)))
  }, [])
  ...
  const resolve = useCallback(async (targets: Campaign[]) => {
    for (const c of targets) {
      ...
      if (cached !== undefined) { write(c.id, cached); continue }
